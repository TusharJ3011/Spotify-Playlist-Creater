...
def get_titles():
    response = requests.get(url="https://bestoftheyear.in/music/hindi/")
    response.raise_for_status()
    html_data = response.text

    soup = BeautifulSoup(html_data, "html.parser")
    data = soup.find_all(name="h4", class_="entry-title", limit=20)
    ...
    create_playlist(title_list, "Bollywood Top 20")
    return
